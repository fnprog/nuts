...
export function Overview() {
  ...
  const valueFormatter = useCallback((value: string) => [`$${value.toLocaleString()}`, "Value"], []);

  const pieChartLabelFormatter = useCallback(({ name, percent }: { name: string; percent: number }) => `${name}: ${(percent * 100).toFixed(0)}%`, []);
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-muted-foreground flex items-center text-xs">{totalDebt > 0 ? ((totalDebt / totalValue) * 100).toFixed(1) : 0}% of value</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            ...
            <div className="flex items-center text-xs text-green-500">
              <ArrowUpRight className="mr-1 h-4 w-4" />${(totalRentalIncome * 12).toLocaleString()} annually
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <Card>
      ...
      <CardContent className="h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} cx="50%" cy="50%" labelLine={true} outerRadius={150} fill="#8884d8" dataKey="value" nameKey="name" label={labelFormatter}>
              {data.map((_, index) => (
