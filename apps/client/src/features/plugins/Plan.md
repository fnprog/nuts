...
3. Server Infrastructure
   ...
   - Migration system using goose (SQL migrations in server/database/migrations/)
     🚫 What's Missing
4. Plugin Metadata Storage - No CRDT collection for plugin metadata
5. Plugin Data Isolation - No namespaced storage for plugin-specific data
6. Plugin Migration System - No way for plugins to register their own migrations
7. Plugin Lifecycle Hooks - No install/uninstall/enable/disable callbacks
8. Server-side Plugin API - No plugin endpoints or domain logic
9. Data Collision Prevention - No namespace strategy for plugin tables/collections
...
- ✅ All plugin tables: plugin*{plugin_id}*{table_name}
- ✅ All plugin indices: idx*plugin*{plugin*id}*{index_name}
...
async up(execute) {
await execute(`         CREATE TABLE IF NOT EXISTS plugin_real_estate_properties (
          id TEXT PRIMARY KEY,
          user_id TEXT NOT NULL,
          name TEXT NOT NULL,
...
async up(execute) {
await execute(`         CREATE TABLE IF NOT EXISTS plugin_real_estate_mortgages (
          id TEXT PRIMARY KEY,
