...
export function RouteComponent() {
  ...
  const filteredPlugins = useMemo(
    () => pluginConfigs.filter((plugin) => plugin.name.toLowerCase().includes(searchTerm.toLowerCase())),
  ...
  return (
    <>
      <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12">
        <div className="flex w-full items-center justify-between gap-2 px-4"></div>
      </header>
      <main className="flex flex-1 overflow-hidden">
        <div className="h-full w-full space-y-8 overflow-y-auto px-6 py-2">
          <div className="space-y-6">
            <div className="flex flex-col justify-between md:flex-row">
              <H2 className="py-4 md:py-0">Plugin Manager</H2>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {filteredPlugins.length > 0 ? (
                    ...
                    <div className="col-span-full py-12 text-center">
                      <P variant="muted">No plugins found</P>
                    </div>
                  )}
                </div>
              </TabsContent>
...
const PluginCard = React.memo(
  ({ pluginConfig, onToggle, onRemove }: { pluginConfig: PluginConfig; onToggle: (id: string, enabled: boolean) => void; onRemove: (id: string) => void }) => {
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant={pluginConfig.enabled ? "default" : "outline"}>{pluginConfig.enabled ? "Enabled" : "Disabled"}</Badge>
          </div>
        ...
        <CardContent>
          <div className="text-muted-foreground">
            <div className="flex justify-between">
              <Small>Version:</Small>
              <Small>{pluginConfig.version}</Small>
            </div>
            <div className="flex justify-between">
              <Small>Author:</Small>
              <Small>{pluginConfig.author}</Small>
            </div>
          </div>
        </CardContent>
        <CardFooter className="flex justify-between">
          <div className="flex items-center gap-2">
            <Switch checked={pluginConfig.enabled} onCheckedChange={() => onToggle(pluginConfig.id, pluginConfig.enabled)} />
            <Small>{pluginConfig.enabled ? "Enabled" : "Disabled"}</Small>
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="icon" className="text-red-500 hover:text-red-600" onClick={() => onRemove(pluginConfig.id)}>
              <X className="h-4 w-4" />
            </Button>
          </div>
        </CardFooter>
