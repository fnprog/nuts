...
import { ComboBox, ComboBoxContent, ComboBoxTrigger } from "@/core/components/ui/combobox";
import { Label } from "@/core/components/ui/label";
import { Button } from "@/core/components/ui/button";
import { ChevronsUpDown } from "lucide-react";
import { P } from "@/core/components/ui/typography";
      ...
      <CardContent className="grid gap-6">
        <div className="grid gap-2">
          ...
          <ComboBox open={isCSelectorOpen} onOpenChange={setIsCSelectorOpen}>
            <ComboBoxTrigger>
              <Button variant="outline" role="combobox" aria-expanded={isCSelectorOpen} className="justify-between">
                {currency ? (
                  <>
                    {currencies.find((c) => c.code === currency)?.name} ({getSymbolFromCurrency(currency)})
                  </>
                ) : (
        ...
        {updatePreferences.isPending && <P className="text-blue-500">{t("common.loading")}</P>}
      </CardContent>
    </Card>
  );
}
