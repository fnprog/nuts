...
import { Field, FieldGroup, FieldError, FieldLabel, FieldSet } from "@/core/components/ui/field";
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-2">
            <FieldSet>
              <FieldGroup>
                <Field>
                  <FieldLabel className="sr-only hidden text-gray-700" htmlFor="name">
                    Username
                  </FieldLabel>
                  <Input id="name" type="text" placeholder="Enter your full name" className="" {...form.register("name")} />
                  {form.formState.errors.name && <FieldError>{form.formState.errors.name.message}</FieldError>}
                </Field>
                <Field>
                  <Button variant="primary" onClick={form.handleSubmit(onSubmit)} disabled={form.formState.isSubmitting}>
                    Continue
                  </Button>
                </Field>
              </FieldGroup>
