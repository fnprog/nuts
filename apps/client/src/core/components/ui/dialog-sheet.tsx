...
import { Dialog, DialogContent, DialogTrigger, DialogHeader, DialogTitle, DialogDescription, DialogFooter, DialogClose } from "@/core/components/ui/dialog";
import { Drawer, DrawerContent, DrawerTrigger, DrawerHeader, DrawerTitle, DrawerDescription, DrawerFooter, DrawerClose } from "@/core/components/ui/drawer";
...
const ResponsiveDialogTrigger = ({ children }: React.ComponentProps<typeof DialogPrimitive.Trigger>) => {
  ...
  return (
    <Fragment key={isMobile ? "mobile" : "desktop"}>
      {isMobile ? <DrawerTrigger asChild>{children}</DrawerTrigger> : <DialogTrigger asChild>{children}</DialogTrigger>}
    </Fragment>
  );
};
...
const ResponsiveDialogDescription = ({ children, className, ...props }: React.ComponentProps<typeof DialogDescription>) => {
  const isMobile = useIsMobile();
