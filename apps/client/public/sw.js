...
const STATIC_CACHE = ["/", "/index.html", "/src/main.tsx", "/src/index.css"];
...
self.addEventListener("fetch", (event) => {
  ...
  if (API_CACHE_PATTERNS.some((pattern) => pattern.test(url.pathname))) {
    event.respondWith(
      Promise.race([fetch(request), new Promise((_, reject) => setTimeout(() => reject(new Error("Network timeout")), 3000))])
        .then((response) => {
